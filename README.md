...
### ファームの導入と更新（本体のボタン操作は不要）

CtmMonitor が ESP32-S3 の ROM ブートローダーを直接話すので、**esptool も Python も
不要**。書き込みモードへの遷移も PC 側から行う（USB-Serial/JTAG のリセット操作）。

- **新品への書き込み・復旧**: 右クリック →「**ATOM にファームを書き込む**」。
  このメニューは普段はグレーで、「デバイスは見えるのに本アプリのファームが応答しない」
  個体（新品・別ファーム・起動不能）を検知したときだけ押せる。導入済みの個体への
  誤書き込みはできない。同梱ファーム（`bin\atom-fw\`。`pio run -d atom` 済みなら
  build.ps1 が自動で詰める）をパーティション表 + アプリ領域に書く。書き込みに失敗して
  起動しなくなった個体もこれで救出できる（実機で、書き込み中の強制終了 → 復旧まで検証済み）
- **更新**: 接続中のデバイスが同梱より古いときだけ、右クリックに
  「**ATOM ファームを更新 (vX → vY)**」が現れる。押したときだけ書き込む
  （**勝手に自動更新はしない**）
- 書き込み中（約 30 秒）は「ATOMS3R の USB ケーブルを抜かないでください」の
  モーダルが最前面に出る。抜くと起動不能になり得るが、その場合も上の復旧手順で戻せる
- **Windows が ATOM を認識しなくなったら**: 右クリック →「**ATOM の接続をリセット（認識しない時）**」。
  USB の登録を削除してドライバを入れ直させる（管理者の許可＝UAC が出る）。実行後に挿し直す

開発者が PlatformIO を使う場合は従来どおり `pio run -d atom -t upload` でもよい。

接続は自動: ポートはレジストリの Espressif (VID_303A) エントリから検出し、
`{"ping":1}` に `ctm-atom` と応答したデバイスだけに接続する（他の機器を荒らさない）。
使うには右クリック →「サブモニタ (ATOM)」を ON にするだけ。
